...
void cRenderer::Render3DObject(cModelObject* pCurrentObject, GLint shaderProgID, cVAOManager* pVAOManager) {
	...
	GLint bIsIsland_UL = glGetUniformLocation(shaderProgID, "bIsIsland");
	glUniform1f(bIsIsland_UL, (float)GL_FALSE);
	GLint bIsSeafloor_UL = glGetUniformLocation(shaderProgID, "bIsSeafloor");
	glUniform1f(bIsSeafloor_UL, (float)GL_FALSE);
	GLint bIsWaterSurface_UL = glGetUniformLocation(shaderProgID, "bIsWaterSurface");
	glUniform1f(bIsWaterSurface_UL, (float)GL_FALSE);
...
void cRenderer::Render3DObjectMovingTexture(cModelObject* pCurrentObject, GLint shaderProgID, cVAOManager* pVAOManager, glm::vec2 offset) {
	...
	glUniform1f(bIsIsland_UL, (float)GL_FALSE);
	GLint bIsSeafloor_UL = glGetUniformLocation(shaderProgID, "bIsSeafloor");
	glUniform1f(bIsSeafloor_UL, (float)GL_TRUE);
	GLint bIsWaterSurface_UL = glGetUniformLocation(shaderProgID, "bIsWaterSurface");
	glUniform1f(bIsWaterSurface_UL, (float)GL_FALSE);
...
void cRenderer::Render3DObjectHeightMap(cModelObject* pCurrentObject, GLint shaderProgID, cVAOManager* pVAOManager, glm::vec2 offset) {
	...
	glUniform1f(bIsSeafloor_UL, (float)GL_FALSE);
	GLint bIsWaterSurface_UL = glGetUniformLocation(shaderProgID, "bIsWaterSurface");
	glUniform1f(bIsWaterSurface_UL, (float)GL_TRUE);

	//Textures
	GLuint ID1 = mpTexture["WaterCaustic"];
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, ID1);

	GLint WaterTexture = glGetUniformLocation(shaderProgID, "WaterTexture");
	glUniform1i(WaterTexture, 1);	// Texture unit 1
	...
	GLuint heightMapID = mpTexture["WaterCaustic"];
...
void cRenderer::RenderTheIsland(cModelObject* pCurrentObject, GLint shaderProgID, cVAOManager* pVAOManager) {
	if (pCurrentObject->isVisible == false)
		return;
	...
	//it is the island
	GLint bIsIsland_UL = glGetUniformLocation(shaderProgID, "bIsIsland");
	glUniform1f(bIsIsland_UL, (float)GL_TRUE);
	GLint bIsSeafloor_UL = glGetUniformLocation(shaderProgID, "bIsSeafloor");
	glUniform1f(bIsSeafloor_UL, (float)GL_FALSE);
	GLint bIsWaterSurface_UL = glGetUniformLocation(shaderProgID, "bIsWaterSurface");
	glUniform1f(bIsWaterSurface_UL, (float)GL_FALSE);
	...
	//Textures
	GLuint ID3 = mpTexture["Rock"];
	glActiveTexture(GL_TEXTURE3);				// Texture Unit 0
	glBindTexture(GL_TEXTURE_2D, ID3);	// Texture now assoc with texture unit 0

	GLuint ID1 = mpTexture["Grass"];
	glActiveTexture(GL_TEXTURE1);	
	glBindTexture(GL_TEXTURE_2D, ID1);
	
	GLuint ID2 = mpTexture["Sand"];
	glActiveTexture(GL_TEXTURE2);	
	glBindTexture(GL_TEXTURE_2D, ID2);	


	GLint Rocktexture_UL = glGetUniformLocation(shaderProgID, "Rocktexture");
	glUniform1i(Rocktexture_UL, 3);	// Texture unit 0

	GLint GrassTexture_UL = glGetUniformLocation(shaderProgID, "Grasstexture");
	glUniform1i(GrassTexture_UL, 1);	// Texture unit 1
	
	GLint SandTexture_UL = glGetUniformLocation(shaderProgID, "Sandtexture");
	glUniform1i(SandTexture_UL, 2);	// Texture unit 1
