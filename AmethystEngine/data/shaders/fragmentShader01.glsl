...
uniform bool bIsSeafloor;
uniform sampler2D WaterTexture;

uniform bool bIsWaterSurface;
...
//				  + ( tex_0_3_ratio.w * tex3_RGB );
	...
	else
	{
		if ( bIsIsland ) 
		{
			if ( fVertWorldLocation.y <= 1.0f )
			{
				discard;
			}
			else if ( fVertWorldLocation.y < 37.0f )
			{
				pixelColour.rgb = texture( Sandtexture, fUVx2.st ).rgb;
				pixelColour.a = 1.0f;				// NOT transparent
			}
			else if( fVertWorldLocation.y >= 37.0f && fVertWorldLocation.y < 47.0f)
			{
				vec3 tex0_RGB = texture( Sandtexture, fUVx2.st ).rgb;
				vec3 tex1_RGB = texture( Grasstexture, fUVx2.st ).rgb;

				float GrassRatio = (47.0f - fVertWorldLocation.y) / 10.0f;

				texRGB = ( GrassRatio * tex0_RGB ) + ( (1.0f-GrassRatio) * tex1_RGB );
				pixelColour.rgb = texRGB.rgb;
				pixelColour.a = 1.0f;				// NOT transparent
			}
			else if( fVertWorldLocation.y > 120.0f && fVertWorldLocation.y <= 140.0f)
			{
				vec3 tex0_RGB = texture( Grasstexture, fUVx2.st ).rgb;
				vec3 tex1_RGB = texture( Rocktexture, fUVx2.st ).rgb;

				float RockRatio = (140.0f - fVertWorldLocation.y) / 20.0f;

				texRGB = ( RockRatio * tex0_RGB ) + ( (1.0f-RockRatio) * tex1_RGB );
				pixelColour.rgb = texRGB.rgb;
				pixelColour.a = 1.0f;				// NOT transparent
			}
			else if (fVertWorldLocation.y > 130.0f)
			{
				pixelColour.rgb = texture( Rocktexture, fUVx2.st ).rgb;
				pixelColour.a = 1.0f;				// NOT transparent
			}
			else
			{
				pixelColour.rgb = texture( Grasstexture, fUVx2.st ).rgb;
				pixelColour.a = 1.0f;				// NOT transparent
			}

			outColour = calcualteLightContrib( pixelColour.rgb, fNormal.xyz, fVertWorldLocation.xyz, specularColour );
		}

		else if (bIsWaterSurface) 
		{
			vec4 newSpecular = specularColour;
			newSpecular.w = 1000000 * (fVertWorldLocation.y - 35.0f) + 10000;

			outColour = calcualteLightContrib( texRGB.rgb, fNormal.xyz, fVertWorldLocation.xyz, newSpecular );
		}
