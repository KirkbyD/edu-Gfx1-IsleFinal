...
int main() {
	...
	/*if (g_vec_pComplexObjects.size() != 0) {
		...
		isObjCtrl = false;
	//}
	FreeCamera.SetEye(glm::vec3(0.0f, 400.0f, 0.0f));
	...
	while (!glfwWindowShouldClose(_window)) {
		...
		for (int index = 0; index != ::g_vec_pGameObjects.size(); index++) {
			...
			if (pCurrentObject->isVisible) {
				if (pCurrentObject->friendlyName == "theIsland") {
					pCurrentObject->recalculateWorldMatrix();
					pRenderer->RenderTheIsland(pCurrentObject, shaderProgID, pTheVAOManager);
				}
				...
				else if (pCurrentObject->friendlyName == "WaterSurface")
				{
		...
		if (!isDevCon) {
			...
			if (mpLight.find("Tracker") != mpLight.end()) {
				glm::vec3 lightDirection = glm::normalize(pFindObjectByFriendlyName("Shark")->getPosition() - mpLight["Tracker"]->getPosition());
				mpLight["Tracker"]->direction = glm::vec4(lightDirection, 1.0f);
			}
